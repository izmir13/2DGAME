...
class Soldier:
    ...
    def __init__(self,team):
        ...
        if Soldier.aim_image == None:
            Soldier.aim_image = load_image('image//Bullet1.png')
        ...
        self.frame = random.randint(0,SoldierState.Idle)
        self.angle = -90
    ...
    def handle_soldier_idle(self):      #가만히 있을때
        self.frame = (self.frame + 1) % self.characterframe
    def handle_soldier_move(self):      #움직일때
        self.fall = True
        if self.jump:
            self.frame = (self.frame + 1) % self.characterframe
            self.y += 3
    ...
    def handle_soldier_aim(self):       #조준일때
        temp = 0        
    ...
    def update(self):
        if self.fall:
            self.falltime += 0.1
            self.y = self.y + (-0.098) * self.falltime * self.falltime
    ...
    def draw(self):
        if self.team == SoldierTeam.Green:
            if self.dir:
                self.greenteam_right_image.clip_draw(self.frame * 60, self.state, 60, 80, self.x, self.y)
            else:
                self.greenteam_left_image.clip_draw(self.frame * 60, self.state, 60, 80, self.x, self.y)
        elif self.team == SoldierTeam.Gray:
            if self.dir:
                self.grayteam_right_image.clip_draw(self.frame * 60, self.state, 60, 80, self.x, self.y)
            else:
                self.grayteam_left_image.clip_draw(self.frame * 60, self.state, 60, 80, self.x, self.y)

        if self.STATE == self.SOLDIER_AIM:
            self.aim_image.clip_draw(36,0,12,12,self.x + 50 * sin(pi * self.angle / 180),self.y + 50 * cos(pi * self.angle / 180))
    ...
    def event(self,event,bullet,wind):
        ...
        if event.type == SDL_KEYUP:     #키를 땠을때
            ...
            if event.key == SDLK_SPACE and self.state == SoldierState.Aim:
                self.charge = False                                          
                bullet.append(Bullet(self.x,self.y,self.power,self.angle,wind))
                self.power = 0
                self.characterframe = SoldierFrame.Idle
                self.state = SoldierState.Idle
                self.STATE = self.SOLDIER_IDLE
